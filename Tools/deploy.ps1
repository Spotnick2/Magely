<#
    deploy.ps1 - Deploy Magely into the WoW: Forever AddOns folder.
    ...
    Magely embeds LibGroupBuffs-1.0. A release gets it from .pkgmeta
    externals; a deploy copies it from the checkout next to this repository
    (../LibGroupBuffs, or -Library) into Magely\Libs\LibGroupBuffs-1.0,
...
function Deploy-Magely {
    $dest = Join-Path $AddOnsPath "Magely"
    ...
    Write-Host "Deploying Magely -> $dest" -ForegroundColor Cyan
    if (-not (Test-Path $dest)) { New-Item -ItemType Directory -Path $dest | Out-Null }

    $files = @("Magely.toc") + (Get-ChildItem -LiteralPath $RepoRoot -Filter "*.lua" |
