...
| Other players' casts: `UNIT_SPELLCAST_SUCCEEDED` | **Registers, but delivered nothing for a group member who was casting.** First run: the watcher ran from 00:45:05 while the owner's second account cast, and `/mprobe report` at 00:45:13 said *"casts: none seen yet"* - no event for any unit at all. (That run was first misread here as "nobody cast"; the owner corrected it.) So the TBC pane's approach - watch everyone's casts - has no replacement on this client: the combat log is forbidden, and this event does not carry other players' casts. |
...
- **Your own casts**: whether `UNIT_SPELLCAST_SUCCEEDED` fires for `player` - and whether its
  `spellID` is readable in combat - is not measured: you did not cast while it was watching. It is
  now the question that matters. If other players' casts cannot be seen, the pane's only source is
  each provider **announcing its own** Innervate / Power Infusion to the group by addon message
  (the TBC pane's `MAGELYCD` sync, minus the combat log), which needs the provider's own cast to be
  observable. Measure it: `/mprobe cast on`, cast anything yourself, out of combat and in a fight,
  then `/mprobe report`.
- **Raid members' casts**: not tried; a party member's were not delivered, so a raid's are unlikely.
