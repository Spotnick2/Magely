<#
    run.ps1 - Run all Magely unit tests.
    ...
    Magely loads LibGroupBuffs-1.0 first (the shared compat layer, engine and
...
# loadfile('Magely.lua') with paths relative to the project.
...
try {
    ...
    # optional: until the host is ported no test loads MagelyConfig.lua or
    # Magely.lua, so this is their only local check.
    ...
    $ownFiles = @(Get-Content (Join-Path $RepoRoot "Magely.toc") |
        Where-Object { $_ -notmatch '^\s*#' -and $_ -match '\.lua\s*$' } |
        ForEach-Object { $_.Trim() })
    if ($ownFiles.Count -eq 0) { Write-Host "Magely.toc lists no Lua files" -ForegroundColor Red; exit 1 }
