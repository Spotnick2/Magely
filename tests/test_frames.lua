...
local function setup(known)
    ...
    H.TeachSpells(known or { "INT_SINGLE", "AMPLIFY", "DAMPEN" })
    T.RefreshSpellData()
...
H.check(T.isMage(), "a mage is recognised at login")
H.check(said(0):find("Loaded", 1, true) ~= nil, "and told Magely loaded: " .. said(0))
H.eq(MagelyDB.visible, true, "the window starts visible by preference")
...
H.eq(MagelyDB.visible, false, "the preference is saved straight away")
WoW.inCombat = false
...
local events = {
    ...
    { "UNIT_AURA", "party1", { addedAuras = { { name = "Arcane Intellect" } } } },
    { "UNIT_AURA", "party1", { addedAuras = { { name = "Amplify Magic" } } } },
...
-- An Amplify or Dampen landing is relevant even with its row hidden: in
-- detect mode it is what makes the row appear.
MagelyDB.amplifyMode = "detect"
H.eq(T.AuraEventIsRelevant("party1", { addedAuras = { { name = "Amplify Magic" } } }), true,
    "an Amplify landing is relevant")
H.eq(T.AuraEventIsRelevant("party1", { addedAuras = { { name = "Renew" } } }), false,
    "somebody else's buff is not")
...
local slash = SlashCmdList["MAGELY"]
H.check(slash ~= nil and SLASH_MAGELY1 == "/magely", "/magely is registered")
for _, cmd in ipairs({ "", "help", "show", "hide", "close", "reset", "config",
                       "options", "settings", "opt", "pos", "garbage" }) do
    local ok, err = pcall(slash, cmd)
    H.check(ok, "/magely " .. (cmd == "" and "<no args>" or cmd) .. ": " .. tostring(err))
end
WoW.flushTimers()
...
at = #WoW.messages
slash("help")
H.check(said(at):find("no Arcane Brilliance known yet", 1, true) ~= nil,
    "without Brilliance, help says left-click is single-target: " .. said(at))
setup({ "INT_SINGLE", "INT_GROUP" })
at = #WoW.messages
slash("help")
H.check(said(at):find("Left-click   Arcane Brilliance", 1, true) ~= nil,
    "with Brilliance, left-click is the group cast: " .. said(at))

-- Reset works while locked, and says the window is still locked.
MagelyDB.lockFrame = true
MagelyDB.pos = { point = "CENTER", relPoint = "CENTER", x = 9999, y = 9999 }
at = #WoW.messages
slash("reset")
H.eq(MagelyDB.pos, nil, "/magely reset works while locked")
...
MagelyDB.pos = nil
at = #WoW.messages
H.check(pcall(slash, "pos"), "/magely pos with no saved position")
...
H.check(pcall(Magely_ApplyAlpha), "Magely_ApplyAlpha runs")
H.check(pcall(Magely_ForceRebuild), "Magely_ForceRebuild runs")
H.check(pcall(Magely_ScheduleRefresh), "Magely_ScheduleRefresh runs")
H.check(pcall(Magely_OnSoloToggle, true), "Magely_OnSoloToggle(true) runs")
H.check(pcall(Magely_OnSoloToggle, false), "Magely_OnSoloToggle(false) runs")
WoW.inCombat = true
H.check(pcall(Magely_ForceRebuild), "Magely_ForceRebuild in combat does nothing, quietly")
H.check(pcall(Magely_OnSoloToggle, true), "and so does the solo toggle")
WoW.inCombat = false
H.check(pcall(WoW.flushTimers), "queued timers run")

------------------------------------------------------------
-- The reagent footer tooltip
--
-- Built only when there is a reagent to show - here, Arcane Powder once
-- Brilliance is known - and it is where a GameTooltip method this client
-- lacks would hide: the stub's catch-all makes an unknown METHOD a silent
-- no-op. So it is run, and what it produces is asserted.
------------------------------------------------------------

setup({ "INT_SINGLE", "INT_GROUP" })
local powder = T.footerButton(17020)
H.check(powder ~= nil, "the Arcane Powder button exists once Brilliance is known")
WoW.clearTooltip()
runScript(powder, "OnEnter")
local tip = WoW.tooltipText()
H.check(tip:find("Item 17020", 1, true), "its tooltip names the item: " .. tip)
H.check(tip:find("in your bags", 1, true), "and how many you have: " .. tip)
H.check(tip:find("Arcane Brilliance", 1, true), "and what consumes it: " .. tip)
runScript(powder, "OnLeave")

WoW.clearTooltip()
WoW.itemsUncached[17020] = true
runScript(powder, "OnEnter")
tip = WoW.tooltipText()
H.check(tip:find("Loading", 1, true), "an uncached item shows a placeholder: " .. tip)
H.check(WoW.itemsRequested[17020], "and the data is requested for next time")
WoW.itemsUncached[17020] = nil

-- Bags changing updates the count.
WoW.itemCounts[17020] = 12
H.check(pcall(WoW.dispatch, "BAG_UPDATE"), "BAG_UPDATE with the footer up")
H.eq(powder.countTxt._text, 12, "and the count follows the bags")
