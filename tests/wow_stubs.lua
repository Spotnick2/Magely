...
-- wow_stubs.lua
--
-- Minimal World of Warcraft: Forever API surface so Magely and the
...
--
-- A copy of LibGroupBuffs' tests/wow_stubs.lua, which models the client's
-- combat refusals most completely. Every change from that copy is marked
-- "Magely:", so the two can be compared and a library fix carried over.
-- Sharing one stub instead is LibGroupBuffs issue #21.
--
...
--     WoW.SetAura("party1", "Arcane Intellect", 1800, 1200)
--
...
function WoW.reset()
    ...
    -- Magely: the player is a Mage (the library stub makes a Priest).
    WoW.SetUnit("player", { name = "Magely Testcase", class = "MAGE", level = 20 })
end
...
-- Magely: the client keeps empty fields - strsplit(",", "a,,b") is "a", "",
-- "b" - and treats every character of `sep` as a delimiter. (Carried over
-- from Wildly's stub; the library's drops empty fields.)
...
local KNOWN_ABSENT = {
    ...
    -- LibStub looks itself up before it exists. (Magely: the library stub also
    -- allows a LibGroupBuffs global; the library creates none, so this does not.)
    LibStub = true,
    -- Magely: its own globals, which start out nil like any others.
    Magely = true, MagelyDB = true, MagelySVCheck = true,
    -- Magely: the hooks Magely.lua defines for the config, which the config
    -- looks up guarded (`if Magely_ForceRebuild then`), so a test that loads
    -- the config alone must be able to read them as nil.
    Magely_ForceRebuild = true, Magely_ApplyAlpha = true, Magely_OnSoloToggle = true,
    Magely_ScheduleRefresh = true,
